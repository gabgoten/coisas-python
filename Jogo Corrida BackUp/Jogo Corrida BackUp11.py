...
while True:
    ...
    if pode_andar == True:
        ...
        if keys[pygame.K_UP] and parou_cima == False:
            personagem_y -= 1
        if keys[pygame.K_UP] and parou_cima == True:
            if Aumento <= 5:
                Aumento += 1
                BarraVerde = 15, 10, Aumento, 50
        if keys[pygame.K_UP]:
    ...
    if (not keys[pygame.K_z] or (not keys[pygame.K_x])) and pygame.time.get_ticks() > tempo_1 + 200 and cima == False:
        Diminuir = Aumento + 1
        if Aumento > 0:
            Aumento -= 1
        if Aumento > 0 and Aumento <= 20:
            Aumento2 -= 1
        if Aumento > 0 and personagem_y < 300:
            personagem_y += 2
        BarraVerde = 15, 10, Aumento, 50
