...
while True:
    ...
    #teclas para andar
    if pode_andar == True and Cansou == False:
        if keys[pygame.K_LEFT] and personagem_x > 0:
            personagem_x -= 1
    ...
    if ((not keys[pygame.K_z] or (not keys[pygame.K_x])) or (keys[pygame.K_z] and (keys[pygame.K_x]))) and pygame.time.get_ticks() > tempo_1 + 700 and cima == False:
        if Aumento > 0:
            Aumento -= 1
